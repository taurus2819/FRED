...
public abstract class DefaultFredExport implements FredRecordExport {
    ...
    /**
     * Returns the age based on the 'known stage' if one is defined or the
     * 'inferred stage' if one is defined, or null
     *
     * @param sample
     * @return
     */
    ...
    /**
     * If there is exactly one adoption record, then returns it, otherwise if
     * there are more than one adoptions, returns the most recent or if all
     * adoption records are undated then returns any. If there are no adoptions,
     * returns null.
     *
     * @throws StorageAccessException
     */
