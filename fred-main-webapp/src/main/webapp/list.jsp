...
List<String>  authRequiredRequests = Arrays.asList(new String[]{
    "folderContent", 
    "document", 
    "sampleId",
    "palList",
...
%><html><head></head><body><%
    ...
    if (listName != null) {
        ...
        %><table><%
            User user = null;
            try {
                if (authRequiredRequests.contains(listName)) {
                    // The  requests that require authentication also happen to
                    // be the ones we really don't want cached
                ...
                %><tr><td>Error: Data entry template out of date, please download the latest version from the FRED website</td><td></td><%
            }      
            if (listName.equals("folderContent") && user != null) {
            ...
            } else if (listName.equals("document") && user != null) {
                ContentProvider provider = new ContentProvider(new File(request.getSession().getServletContext().getRealPath("/content")));
            ...
            } else if (listName.equals("sampleId") && user != null) {
                Sample sample = null;
                try {
            ...
            } else if (listName.equals("palList") && user != null) {
                try {
            ...
            } else if (listName.equals("oldPalList") && user != null) {
                try {
                    UserFolder userFolder = folderUtil.getUserFolder(Integer.parseInt(request.getParameter("folderID")), user);
                    Feature[] features = featureUtil.getFeaturesInFolder(userFolder);
                    if (features.length > 0) {
                        for (int i = 0; i < features.length; i++) {
                            List<? extends Paleontology> palRecords = recordUtil.getPaleontologyRecords(features[i]);
                    ...
                    e.printStackTrace();
                    %><tr><td>Error: failed to retrieve Pal list</td><td></td></tr><%
                }
            } else if (listName.equals("blankPalList")) {
                    %><tr><td>No records defined</td><td>-1</td></tr><%
            } else if (listName.equals("locSampleList") && user != null) {
                try {
                    UserFolder userFolder = folderUtil.getUserFolder(Integer.parseInt(request.getParameter("folderID")), user);
                    Feature[] features = featureUtil.getFeaturesInFolder(userFolder);
                    if (features.length > 0) {
                ...
                %><tr><td><%=FeatureUtil.getFeatureIdentifyingName(features[i])
                    ...
                    e.printStackTrace();
                    %><tr><td>Error: failed to retrieve location sample list</td><td></td></tr><%
                }
            } else if (listName.equals("blankLocSampleList")) {
			%><tr><td>No samples defined</td><td>-1</td></tr><%
            } else if (listName.equals("sampleList") && user != null) {
                try {
                    ...
                    e.printStackTrace();
                    %><tr><td>Error: failed to retrieve sample list</td><td></td></tr><%
                }
            } else if (listName.equals("blankSampleList")) {
                %><tr><td>No samples defined</td><td>-1</td></tr><%
            } else if (listName.equals("localityList") && user != null) {
                try {
                    String folderId = request.getParameter("folderID");
                    UserFolder userFolder = folderUtil.getUserFolder(Integer.parseInt(folderId), user);
                    Feature[] features = featureUtil.getFeaturesInFolder(userFolder);
                    ...
                    e.printStackTrace();
                    %><tr><td>Error: failed to retrieve locality list</td><td></td></tr><%
                }
            } else if (listName.equals("blankLocalityList")) {
                %><tr><td>No features defined</td><td>-1</td></tr><%
            } else if (listName.equals("folderList") && user != null) {
                try {
