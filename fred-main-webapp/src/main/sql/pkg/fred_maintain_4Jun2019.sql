...
AS
  ...
  BEGIN
    ...
    FOR CRow IN Email_Cur LOOP
      ...
      END IF;
      SYS.SEND_MAIL(EmailAdd, 'FRED Database - localities to approve/reject', EmailString, 'Fossil_Record_Administrator');
    END LOOP;
    COMMIT;
exception
   when others then
...
*/
    ...
    FOR CRow IN Feat_Cur LOOP
	 	begin
			...
			if kount > 49 then
		    	...
		    	SYS.SEND_MAIL(UserEmail, 'FRED Database - localities approved/rejected', EmailString, 'Fossil_Record_Administrator');
				kount := 0 ;
	   	end if ;
	  exception
			when others then
  ...
  BEGIN
    ...
    SYS.SEND_MAIL(PanelistEmail, 'FRED Database - new taxonomic names', EmailString, 'Fossil_Record_Administrator');
  ...
  BEGIN
    ...
    FOR CRow IN Taxa_Cur LOOP
      ...
      END IF;
    END LOOP;
    SYS.SEND_MAIL(UserEmail, 'FRED Database - taxonomic names approved/rejected', EmailString, 'Fossil_Record_Administrator');
  ...
  BEGIN
    ...
    FOR CRow IN Pal_Cur LOOP
      EmailString := EmailString || CRow.FR_Number || CHR(10);
    END LOOP;
    SYS.SEND_MAIL(UserEmail, 'FRED Database - confidentiality about to lapse', EmailString, 'Fossil_Record_Administrator');
