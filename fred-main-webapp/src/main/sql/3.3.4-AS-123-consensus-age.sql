...
from (
        ...
        left join age a_lower on (a_lower.age_id = s.age_lower_id and a_lower.age_id not in (153, 154))
        left join age a_upper on (a_upper.age_id = s.age_upper_id and a_upper.age_id not in (153, 154))
...
from (
        ...
        left join age a2_lower on (a2_lower.age_id=s2.age_lower_id and a2_lower.age_id not in (153, 154))
        join age a2_upper on (a2_upper.age_id=s2.age_upper_id and a2_upper.age_id not in (153, 154))
) union (
        ...
        left join age a3_lower on (a3_lower.age_id=s3.age_lower_id and a3_lower.age_id not in (153, 154))
        left join age a3_upper on (a3_upper.age_id=s3.age_upper_id and a3_upper.age_id not in (153, 154))
))
group by sample_id
 ;
