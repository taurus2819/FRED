...
%><%
	...
	String foldID, featID, recID, featType, featName, sampName, auditID, drillSampName, locStatus, query, recType, imageName;
	int userID = user.getPersonId(), userRights, wRecordCount, sampID;
	...
	if (request.getParameter("ID") != null) {
		...
		userRights = folder.getUserRights();
		...
		if (folder.isAllowedReadLocalities()) {
			...
			for (Iterator i = folder.getAsVector(Folder.FEATURES).iterator(); i.hasNext(); ) {
				...
				for (Iterator j = feature.getAsVector(Feature.SAMPLES).iterator(); j.hasNext(); ) {
					...
					wRecordCount = sampleHeader.getWorkingRecordCount();
					if (wRecordCount > 0) {
						sampID = sampleHeader.getSampleID();
						if (sampCount > 1) {
							out.print("<tr><td width='20'><img src='images/drill.gif' height='20' width='20' /></td><td colspan='4' class='heading'>" + sampleHeader.getDrillholeDepth() + "</td>");
