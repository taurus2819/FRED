...
try {
	...
	if (panelList.size() > 0) {
		...
		for (Iterator i = panelList.iterator(); i.hasNext(); ) {
			...
			%><tr><td style="text-align: left"><a href="taxa_group_detail.jsp?ID=<%=group.getGroupId()%>" class="heading"><%=group.getName()%></a>&nbsp;&nbsp;</td>
			...
			<td style="text-align: left"><a href="taxa_panelist.jsp?GroupID=<%=group.getGroupId()%>" title="Edit Users"><img src="images/prefs.gif" border="0" height="20" width="20" /></a></td></tr><%
		}
		%></table><%
		endDETable(pageContext);
	}
