		...
		try {
			...
			if (feature.getSamples() != null) {
				if (feature.getSamples().size() > 1) {
					...
					response.sendRedirect("detail_hib.jsp?ID=" + ((Sample)feature.getSamples().iterator().next()).getSampleId());
				}
			} else {
				response.sendRedirect("drillhole_detail.jsp?ID=" + featID);
			}
		...
		if (featureUtil.isAllowedReadFeatureSite(user, feature)) {
			...
			if (isAllowedReadFeature) {	
				...
				if ((new FolderUtil(factory)).getPersonalFolders(user).size() > 0) {
					...
					<tr><td>You can add this locality to one of your personal folders by selecting it from the list and clicking <i>Add</i>.  You may wish to do this to allow you to add <%=((featType.equals(FREDConstants.OUTCROP)) ? "" : "samples or ")%>adoption/paleontology records.</td></tr>
				...
				if (featureUtil.isAllowedApproveFeature(user, feature)) {
					...
					System.out.println("next frnumber = " + frNumber.getFrId());
					%><p><%
					startDETable(pageContext);
					%><table border="0" width="160">
