...
function checkDrill() {
	with (document.sampForm) {
		if ((SampID.value != "-" && (TopDepth.value != "" || BottomDepth.value != "" || DrillType.value != "")) || (SampID.value == "-" && TopDepth.value == "" && BottomDepth.value == "" && DrillType.value == "-")) { //check that either a sample is selected or new data is entered BUT not both
...
<%	if (request.getParameter("FeatID") != null) {
		...
		out.println("<tr><td colspan='2' class='bigheading' align='center'>Drillhole: " + rs.getString(1) + "</td></tr>");
