...
%><%
	...
	String sampID, recID, featType, status = "", query, allView = "";
	int i = 1, userRights = 0, execUp;
	...
	//check status of locality.  If not approved (or user not logged in) then use FR account, otherwise use user account
	query = "SELECT Status FROM Sample_All_View WHERE Sample_ID = ?";
	...
	if (rs.next()) {
		if (rs.getString(1).equals("approved")) {
			if (user !=  null) {
				...
				connection = JspUtils.createDatabaseConnection(session, CONNECTION, DB_NAME, application);
				out.println("Logged in as FR");
			}
			userRights = 1;
		} else {
			connection = JspUtils.createDatabaseConnection(session, CONNECTION, DB_NAME, application);
			...
			if (user != null) {
				...
				query = "SELECT Status, User_Rights FROM Sample_Security_View WHERE Sample_ID = ? AND User_ID = ?";
				doubleData[0] = new Integer(Integer.parseInt(sampID));
				doubleData[1] = new Integer(user.getPersonId());
				rs = frConn.executeQuery(query, doubleTypes, doubleData);
			...
			out.println("Logged in as FR - record not approved.  Rights = " + userRights);
		}
	} else {
		connection = JspUtils.createDatabaseConnection(session, CONNECTION, DB_NAME, application);
		userRights = 0;
	...
	if (sampID != null && (userRights & 1) != 0) {
		...
		if (featType.equals("Outcrop")) {
			...
			query = "SELECT Sample_ID, Sample_Name, Drillhole_Depth FROM Sample_" + allView + "View WHERE Feature_ID = ? AND Top_Depth IS NOT NULL ORDER BY Top_Depth";
		...
		query = "SELECT Record_ID, Collection_Date, Date_Rounding, Strat_Unit, In_Place, Not_Collected, Significance, Inferred_Stage, Known_Stage, Column_Map, Dip, Dip_Direction, Strike, Facing, Grainsize, Comparator_Used, Bed_Thickness, Bedding, Weathering, Hardness, Carbonate, Colour, Deposition_Env, Rock_Nature, Correspondence, Collector_ID, Sent_To_Fossil_Group_ID, Sed_Feature_ID FROM FR.Sample_Property_" + allView + "View WHERE Sample_ID = ?";
		data[0] = new Integer(Integer.parseInt(sampID));
		rs = connection.executeQuery(query, types, data);
		preserveStatement = connection.preservePreparedStatement();
		if (rs.next()) {
...
*/			out.println("<tr><td><img src='images/blank.gif' height='30' width='1' /></td></tr>");
		...
		//Adoption
		query = "SELECT Record_ID, Adoption_Date, Date_Rounding, Adopted_Stage, Comments FROM FR.Adoption_" + allView + "View WHERE Sample_ID = ?";
		data[0] = new Integer(Integer.parseInt(sampID));
		rs = connection.executeQuery(query, types, data);
		preserveStatement = connection.preservePreparedStatement();
		while (rs.next()) {
...
*/			out.println("<tr><td><img src='images/blank.gif' height='30' width='1' /></td></tr>");
		...
		//Paleontology
		query = "SELECT DISTINCT Record_ID, Identification_Date, Date_Rounding, Stage, Stage_Comments, Lab, Lab_Number, Collection_Comments FROM FR.Paleontology_" + allView + "View WHERE Sample_ID = ?";
		data[0] = new Integer(Integer.parseInt(sampID));
		rs = connection.executeQuery(query, types, data);
		preserveStatement = connection.preservePreparedStatement();
		while (rs.next()) {
