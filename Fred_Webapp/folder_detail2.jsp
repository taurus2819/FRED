...
%><%
	...
	String foldID, featID, recID, featType, featName, sampID, sampName, auditID, drillSampName, locStatus, query, recType, imageName;
	int userID = user.getPersonId(), userRights, wRecordCount;
		...
		if (folder.isAllowedReadLocalities()) {
			...
			for (Iterator i = folder.getAsVector(Folder.FEATURES).iterator(); i.hasNext(); ) {
				...
				for (Iterator j = feature.getAsVector(Feature.SAMPLES).iterator(); j.hasNext(); ) {
					sample = new Sample(((Integer) j.next()).intValue(), user, state);
