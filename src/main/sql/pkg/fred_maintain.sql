CREATE OR REPLACE PACKAGE FRED_Maintain
AUTHID CURRENT_USER
AS
  PROCEDURE Go;
...
END FRED_Maintain;
/
CREATE OR REPLACE PACKAGE BODY FRED_Maintain
AS
  PROCEDURE Go
  AS
  BEGIN
  ...
  BEGIN
    ...
    FOR CRow IN Email_Cur LOOP
      ...
      END IF;
      SYS.SEND_MAIL(EmailAdd, 'FRED Database - localities to approve/reject', EmailString);
    END LOOP;
    COMMIT;
  ...
  BEGIN
    ...
    END IF;
    EmailString := 'Your samples have been approved or rejected as listed below:' || CHR(10) || CHR(10) ||
     'Feature Name' || CHR(9) || 'Masterfile' || CHR(9) || 'FR Number' || CHR(9) || 'Action' || CHR(9) || 'Comments' || CHR(10);
    FOR CRow IN Feat_Cur LOOP
      EmailString := EmailString || CRow.Feature_Name || CHR(9) || CRow.Name || CHR(9) || CRow.FR_Number || CHR(9) ||
       CRow.Type || CHR(9) || CRow.Curator_Comments || CHR(10);
    END LOOP;
    EmailString := EmailString || CHR(10) || 'Click here to view your folders http://www.fred.org.nz/folder_list.jsp';
    SYS.SEND_MAIL(UserEmail, 'FRED Database - localities approved/rejected', EmailString);
  ...
  BEGIN
    ...
    SYS.SEND_MAIL(PanelistEmail, 'FRED Database - new taxonomic names', EmailString);
  ...
  BEGIN
    ...
    FOR CRow IN Taxa_Cur LOOP
      ...
      END IF;
    END LOOP;
    SYS.SEND_MAIL(UserEmail, 'FRED Database - taxonomic names approved/rejected', EmailString);
  ...
  BEGIN
    ...
    FOR CRow IN Pal_Cur LOOP
      EmailString := EmailString || CRow.FR_Number || CHR(10);
    END LOOP;
    SYS.SEND_MAIL(UserEmail, 'FRED Database - confidentiality about to lapse', EmailString);
