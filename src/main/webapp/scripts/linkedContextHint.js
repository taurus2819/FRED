 ...
 */
LinkedContextHint = function(activatorField, linkedField, urlGenerator, blurCode, omitFieldFix) {
    ...
    //Store 
    linkedContextHints[this.key] = this;
    
    //Setup the field
    if (blurCode) {
	    eval("activatorField.onblur = function() {getLinkedContextHint('" + this.key + "').hideDropout();" + blurCode + "};");
	} else {
	    eval("activatorField.onblur = function() {getLinkedContextHint('" + this.key + "').hideDropout();};");
	}
	LinkedContextHint.addEvent(this.field, "keyup", LinkedContextHint.processUnknownKeyStroke);
	//eval("activatorField.onkeyup = function() {getLinkedContextHint('" + this.key + "').processKeyStroke();};");
...
this.processHint = function() {
	var linkedVal = this.linkedField.value;
...
this.createAndShowTable = function(xml) {
	...
	for (var i=0; i<names.length; i++) {
		...
		eval("cell.onclick = function() {getLinkedContextHint('" + this.key + "').updateField(\"" + hint + "\");}");
		eval("cell.onmouseover = function() {getLinkedContextHint('" + this.key + "').highlightMouse(" + i + ");}");
		eval("cell.onmouseout = function() {getLinkedContextHint('" + this.key + "').clearHighlight(" + i + ");}");
...
LinkedContextHint.addEvent = function(element, evname, func) {
...
var linkedContextHints = new Object();

//Global/static method to find a context hint
function getLinkedContextHint(id) {
...
LinkedContextHint.processUnknownKeyStroke = function(event) {
	//For ie....
	(this.isIE) && (event = window.event);
	
	var keyCode = (document.layers) ? keyStroke.which : event.keyCode;
	
	getLinkedContextHint(((event.srcElement) ? event.srcElement : event.target).id).processKeyStroke(keyCode);
}